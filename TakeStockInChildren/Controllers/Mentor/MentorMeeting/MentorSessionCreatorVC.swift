...
class MentorSessionCreatorVC: BaseViewController, UITextFieldDelegate,UITextViewDelegate {
    ...
    @IBAction func checkNoShow(_ sender: Any) {
        if !noShowSelected {
            checkBtn.setImage(UIImage(named: "checkmark")?.withRenderingMode(.alwaysTemplate), for: .normal)
            noShowSelected = true
            ...
            checkBtn.setImage(UIImage(named: "")?.withRenderingMode(.alwaysTemplate), for: .normal)
    ...
    @IBAction func btnSelectDate(_ sender: Any){
       ...
       let result = formatter.string(from: date)
        ...
        alert.addDatePicker(mode: .date, date: Date(), minimumDate:nil , maximumDate: Date()) { date in
            ...
            let strD ate = dateFormatter.string(from: date)
            print("strdate",strDate)
            self.txtDate.text = strDate
