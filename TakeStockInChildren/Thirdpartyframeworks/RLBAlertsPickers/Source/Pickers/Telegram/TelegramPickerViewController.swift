...
final public class TelegramPickerViewController: UIViewController {
    ...
    var preferredTableHeaderHeight: CGFloat {
        switch mode {
        case .normal: return TelegramPickerViewController.UI.maxHeight / TelegramPickerViewController.UI.multiplier + TelegramPickerViewController.UI.insets.top + TelegramPickerViewController.UI.insets.bottom
        case .bigPhotoPreviews: return TelegramPickerViewController.UI.maxHeight + TelegramPickerViewController.UI.insets.top + TelegramPickerViewController.UI.insets.bottom
    ...
    fileprivate lazy var collectionView: UICollectionView = { [unowned self] in
        ...
        $0.contentInset = TelegramPickerViewController.UI.insets
    ...
    fileprivate lazy var layout: PhotoLayout = { [unowned self] in
        $0.delegate = self
        $0.lineSpacing = TelegramPickerViewController.UI.minimumLineSpacing
        return $0
        }(PhotoLayout())
    ...
    fileprivate lazy var tableView: UITableView = { [unowned self] in
        $0.dataSource = self
        $0.delegate = self
        $0.rowHeight = TelegramPickerViewController.UI.rowHeight
    ...
    func sizeForAsset(asset: PHAsset) -> CGSize {
        switch mode {
        case .bigPhotoPreviews:
            let minValue: CGFloat = TelegramPickerViewController.UI.maxHeight / TelegramPickerViewController.UI.multiplier
            var size = CGSize.init(width: asset.pixelWidth, height: asset.pixelHeight)
            let multiplier = TelegramPickerViewController.UI.maxHeight / size.height
            size.height *= multiplier
            size.width = max(minValue, size.width*multiplier)
            return size
        case .normal:
            let value: CGFloat = TelegramPickerViewController.UI.maxHeight / TelegramPickerViewController.UI.multiplier
    ...
    func sizeForItem(item: StreamItem) -> CGSize {
        switch item {
        case .camera:
            let side = TelegramPickerViewController.UI.maxHeight / TelegramPickerViewController.UI.multiplier
