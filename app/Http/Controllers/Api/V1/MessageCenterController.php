...
class MessageCenterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $all_mentee_data = DB::table('message_center')->where('is_mentee',1)->where(function ($query)               {
                                $query->where('created_by', '=', 1)
                                      ->orWhere('created_by', '=', $this->assigned_by);
                            })->get()->toarray();
    	// $all_mentee_data = DB::table('message_center')->where('is_mentee',1)->where('created_by',1)->orWhere('created_by',$this->assigned_by)->get()->toarray();
        $custome_mentee_data = DB::table('message_center_users')->where('user_type','mentee')->where('user_id', $this->user_id)->get()->toarray();

        if(!empty($all_mentee_data)){
            foreach($all_mentee_data as $d){
                $a_ids[] = $d->id;
            }
        }
        if(!empty($custome_mentee_data)){
            foreach($custome_mentee_data as $d){
                $c_ids[] = $d->message_id;
            }
        }
        // echo $this->assigned_by;
        // echo '<pre>'; print_r($all_mentee_data);
        // echo '<pre>'; print_r($custome_mentee_data);
        // die;
    ...
    public function index_bkp(Request $request)
    {
        ...
        $data = DB::table('message_center')->selectRaw("id,message,DATE_FORMAT(created_at,'%D %b,%Y %H:%i %p') AS created_at")->where('is_mentee', 1)->take($take)->skip($skip)->orderBy('id','desc')->get()->toarray();
        $count_data = DB::table('message_center')->where('is_mentee', 1)->count();
