...
//        }
    ...
    {
        Log::debug("generate_room request: ".print_r($request->all(), true));
    ...
    public function disconnect_room(Request $request)
    {
        ...
        //     // $message = "Video call denied from ".$receiver_data->firstname;
        //     $message = "Video call has been cancelled";
        //     $time = time();
        //     $web_send_data = [
    ...
    {
        Log::debug("denied_call request all: ".print_r($request->all(), true));
        $room_sid = '';
        $unique_name = !empty($request->unique_name) ? $request->unique_name : '';
        // $denied_by = !empty($request->denied_by) ? $request->denied_by : '';
        $denied_by_type = !empty($request->denied_by_type) ? $request->denied_by_type : 'mentee';
        ...
        $denied_by = !empty($request->denied_by) ? $request->denied_by : $video_chat_rooms->receiver_id;
        ...
        if (isset($receiver_data) && $receiver_data->web_fcm_token) {
            ...
            // Log::debug("web_fcm_token: ". print_r($receiver_data->web_fcm_token, true));

            $web_fields = array('to' => $receiver_data->web_fcm_token, 'data' => $web_send_data, 'priority' => "high");
            $result = sendPushNotificationWithV1($web_fields);
            // Log::debug("denied push: ". print_r($result, true));
        }
    ...
    private function ios_voip_push_connect($voip_device_token, $uuid, $receiver_accesstoken, $receiver_id, $receiver_type, $unique_name, $sender_name, $room_sid, $remaining_time)
    {
        ...
        if (!empty($voip_device_token)) {
            ...
            //     'title' => "Incoming Video call", 
            //     'type' => 'video_chat', 
            //     'receiver_accesstoken' => $receiver_accesstoken, 
            ...
            if (curl_errno($ch)) {
                ...
                Log::error("APNs: Message not delivered. Call Connect Error: ".curl_error($ch));
            } else {
                Log::info("APNs: Message delivered. Call Connect");
    ...
    public function iosVoipTest(Request $request)
    {
        ...
        if (curl_errno($ch)) {
            // echo 'Error: ' . curl_error($ch);
            return Response::json(['status' => false, 'message' => "Notification not delivered.", 'error' => curl_error($ch)]);
        } else {
            // echo 'Response: ' . $response;
            return Response::json(['status' => true, 'message' => "Notification delivered.", 'response' => $response, 'payload' => json_decode($payload)]);
        }
    ...
    private function ios_voip_push_disconnect($voip_device_token, $uuid, $receiver_id, $receiver_type, $unique_name, $sender_name, $room_sid)
    {
        ...
        if (!empty($voip_device_token)) {
            ...
            Log::debug("Disonnect payload: ".print_r($payload, true));
            ...
            if (curl_errno($ch)) {
                // echo 'Error: ' . curl_error($ch);
                Log::error("APNs: Message not delivered. Call Disonnect Error: ".curl_error($ch));
            } else {
                Log::info("APNs: Message delivered. Call Disonnect");
    ...
    private function ios_voip_push_denied($voip_device_token, $uuid, $receiver_id, $receiver_type, $unique_name, $sender_name, $room_sid)
    {
        ...
        if (!empty($voip_device_token)) {
            ...
            //     'data' => $data_arr
            // ]);
            Log::debug("Denied payload: ".print_r($payload, true));
            ...
            if (curl_errno($ch)) {
                // echo 'Error: ' . curl_error($ch);
                Log::error("APNs: Message not delivered. Call Denied Error: ".curl_error($ch));
            } else {
                Log::info("APNs: Message delivered. Call Denied");
