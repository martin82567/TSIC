...
class MenteeLoginControllerBackup extends Controller
{
    ...
    public function login(Request $request)
    {
      ...
      $check_user = DB::table('mentee')->select('mentee.id','mentee.email','mentee.password','mentee.status','student_status.view_in_application','mentee.platform_status','mentee.assigned_by')->leftJoin('student_status','student_status.id','mentee.status')->where('mentee.platform_status',1)->where('mentee.email', $request->email)->orderBy('mentee.created_at', 'desc')->first();
