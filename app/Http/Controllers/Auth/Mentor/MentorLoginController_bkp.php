...
class MentorLoginControllerBackup extends Controller
{
    ...
    public function login(Request $request)
    {
      ...
      $check_user = DB::table('mentor')->select('mentor.id','mentor.email','mentor.password','mentor.is_active','mentor_status.view_in_application','mentor.platform_status','mentor.assigned_by')->leftJoin('mentor_status','mentor_status.id','mentor.is_active')->where('mentor.email', $request->email)->where('mentor.platform_status',1)->orderBy('mentor.created_at', 'desc')->first();
