...
class UniversalLoginController extends Controller
{
    ...
    {
        // Session END handle
        // session(['message' => 'The Take Stock App is currently down for summer maintenance and enhancement updates. The app will return in August prior to the Fall semester.', 'msg_class' => 'danger']);
    ...
    public function logout()
    {
        if(Auth::guard('mentor')){
            // // Web FCM token set blank
            // DB::table('mentor')->where('id', Auth::guard('mentor')->user()->id)->update(['web_fcm_token' => '']);
...
//            Log::info("mentor is logged out");
        ...
        if(Auth::guard('mentee')){
            // // Web FCM token set blank
            // DB::table('mentee')->where('id', Auth::guard('mentee')->user()->id)->update(['web_fcm_token' => '']);
