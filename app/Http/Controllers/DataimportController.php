    ...
    public function mentee_mentor_data_import()
    {
        ...
        if (!empty($mentee->toarray())) {
            foreach ($mentee as $student) {
                $studentList = collect(getStudentMatchForMentor($student->externalId, $token, $baseURL));
                    ...
                    foreach ($studentList as $assign_mentee) {
                        if ($assign_mentee->mentorAssignmentTypeID == 1) {
                            ...
                            if (!empty($mentor_exist)) {
                                $activeMentorIDs[] = $mentor_exist->id;
                                ...
                                if (empty($assign_mentee_exist)) {
                                    $inserted_data_assign = array('mentee_id' => $mentee_id, 'assigned_by' => $mentor_exist->id, 'created_date' => $created_date, 'is_primary' => !empty($assign_mentee->isPrimary) ? 1 : 0);
...
//                        }
                ...
                $mentorsToRemove = array_diff($currentAssignedMentors, $activeMentorIDs);

                if (!empty($mentorsToRemove)) {
                    foreach ($mentorsToRemove as $mentor_id) {
                        $assign_mentee_exist = DB::table('assign_mentee')
                            ->select('*')
                            ->where('mentee_id', $mentee_id)
                            ->where('assigned_by', $mentor_id)
                            ->first();
                ...
                DB::table('mentee')->where('id', $mentee_id)->update(['is_inserted' => 1]);
            }
    ...
    public function syncAgencyPrograms()
    {
