...
class GoalController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($type == 'pending') {
            $data = DB::table('goaltask')
                ...
                ->where('assign_goal.victim_id', Auth::user()->id)
                ->where('goaltask.status', 1)
                ->where('goaltask.type', 'goal')
        ...
        return view('mentee.my-goals')->with('type', $type)->with('data', $data);
    ...
    public function show(Request $request)
    {
        ...
        $assign_goal = DB::table('assign_goal')->select('*')->where('goaltask_id', $goal_id)->first();
        $goal_status = $assign_goal->status;
//        dd($goal_status);
        $goal_files = DB::table('goaltaskuserfiles')->select('*')->where('goaltask_id', $goal_id)->get();

        $base_path = Storage::disk('s3')->url('goaltask');

        return view('mentee.my-goals-detail')->with('detail', $goal_detail)->with('notes', $notes)->with('goal_status', $goal_status)->with('files', $goal_files)->with('base_path', $base_path);
    ...
    public function saveNote(Request $request, $id)
    {
        ...
        if ($type == "goal") {
            ...
            session(['success_message' => 'Goal Notes saved successfully']);

            return redirect()->back();

        } else {
            session(['error_message' => 'Goal Notes are not saved.']);
        }
        return redirect()->back();
...
//        dd($request->file());
            ...
            if (!empty($request->files)) {
                ...
                foreach($request->files as $all_files) {
                    for ($i = 0; $i < count($all_files); $i++) {
                        ...
                        session(['success_message' => 'Goal Files saved successfully']);

                    }
                }} else {
                session(['error_message' => 'Goal Files Not saved']);
            }
    }
            return redirect()->back();
    ...
    public function actiongoaltask(Request $request, $id)
    {
        ...
        $assign_goal = DB::table('assign_goal')->select('*')->where('goaltask_id', $id)->first();
    ...
    public function filedeletegoaltask(Request $request,$goaltaskuserfiles_id)
    {
        ...
        session(['success_message' => 'Goal File deleted successfully']);
