...
function createSession($session, $token, $baseURL)
{
    ...
    try {
        ...
        // Log::debug("token: ".print_r($token, true));
        // Log::debug("baseURL: ".print_r($baseURL, true));
        // Log::debug("json: ".print_r($session, true));
        ...
        Log::debug("CreateSession helpers Error: ".print_r($e->getMessage(), true));
    }
}
...
function sendPushNotificationWithV1($payload)
{
    ...
    $firebase = new FCMServiceProvider(base_path('storage/app/configs/firebase.json'));

    try {
        $result = $firebase->sendNotification($payload);
        // Log::debug("fcm response:: ". print_r($result, true));
        
    } catch (Exception $e) {
        Log::debug("FCM ERROR: ". print_r($e->getMessage(), true));
        $result = null;
    }
