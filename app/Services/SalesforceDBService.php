    ...
    public function authenticate()
    {
        ...
        try {
            ...
            if (!empty($settings->salesforce_token) && !empty($settings->base_url)) {
                if (isset($settings->updated_at) && Carbon::parse($settings->updated_at)->diffInMinutes(Carbon::now()) < 60) {
                    return [
                        'success' => true,
...
//        Log::info(json_encode($sessionObject));
            ...
            Log::debug("SalesForce API Response 2: ".print_r($response, true));
...
//                Log::info('Student ID::' . $student->Id . "....Status..." . $student->npsp__Primary_Affiliation__c);
    ...
    public function getStudentMentorMatch()
    {
        $settings = DB::table('settings')->where('id', 1)->first();
        $date = $settings->successful_updated_at;
        if (empty($date)) {
            $date = Carbon::now()->subDay(7)->toIso8601ZuluString();
        }
