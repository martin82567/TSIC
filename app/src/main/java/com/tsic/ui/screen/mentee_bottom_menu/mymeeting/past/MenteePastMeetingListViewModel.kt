...
class MenteePastMeetingListViewModel(private val fragment: MenteePastMeetingListFrag) {

    var listPastMeeting = ObservableField<List<Meeting>>(emptyList())
                    ...
                    { result ->
                        if (result.status == Status.SUCCESS) {
                            ...
                            fragment?.binding?.noSessionFound?.visibility =
                                if (result.data?.meeting?.size == 0) View.VISIBLE else View.GONE
