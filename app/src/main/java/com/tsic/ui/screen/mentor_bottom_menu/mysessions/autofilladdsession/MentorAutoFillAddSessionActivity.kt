package com.tsic.ui.screen.mentor_bottom_menu.mysessions.autofilladdsession
...
import com.tsic.databinding.ActivityMentorAutofillAddSessionBinding
...
class MentorAutoFillAddSessionActivity : AppCompatActivity() {
    ...
    private val binding by lazy {
        DataBindingUtil.setContentView<ActivityMentorAutofillAddSessionBinding>(
            this,
            R.layout.activity_mentor_autofill_add_session
        )
    }
    ...
    private fun initUiAndListeners() {
        binding.vm = MentorAutoFillAddSessionViewModel(this)
        ...
        binding?.vm?.also {
            it.title.set(intent?.getStringExtra("agenda_name"))
            it.description.set(intent?.getStringExtra("description"))
            it.menteeName.set(intent?.getStringExtra("mentee_name"))
            it.menteeId= intent?.getStringExtra("mentee_id").toString()
            it.meetingId= intent?.getStringExtra("meeting_id").toString()
            it.date.set(intent?.getStringExtra("date"))
            it.sessionMethodLocationValue.set(intent?.getStringExtra("session_method_location_id"))
            it.sessionMethodLocationKey.set(intent?.getStringExtra("session_method_location"))
        }
    ...
    fun sessionType() {
        val sessionType: Map<String, String> =
            mapOf("Group" to "1", "Individual" to "2")
        val list = arrayListOf<String>()
