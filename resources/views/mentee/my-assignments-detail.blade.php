                    ...
                    <div class="col-lg-6">
                        @if($goal_status === 1 || $goal_status === 0)
                            <div class="modal fade" id="addNotes" tabindex="-1" role="dialog" aria-labelledby="addNotes" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <form role="form" action="{{ url('/mentee/assignment_note/save/' . $detail->id )}}" method="post" enctype="multipart/form-data" id="save_form">
                                            {{ csrf_field() }}
                            ...
                            <div class="modal fade" id="addFiles" tabindex="-1" role="dialog" aria-labelledby="addNotes" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <form role="form" action="{{ url('/mentee/assignment_files/save/' . $detail->id )}}" method="post" enctype="multipart/form-data" id="save_filesForm">
                                            {{ csrf_field() }}
                                            <div class="modal-header">
            ...
            <div class="box-inner">
                <div class="form-section">
                                ...
                                @foreach($files as $d)
                                    <div class="thumbnail col-md-2">
                                        <a href="{{ $base_path. '/' . $d->file_name}}" target="_blank" class="d-inline-block">
                                            ...
                                            <a class="delete" onclick="return confirm('Are you sure, you want to delete this image?')" href="{{ url('/mentee/assignment_files/delete/' . $d->id )}}" title="Delete Image"><i class="fa fa-minus pl-1"></i></a>
                                        </a>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="box-footer">
                        <form method="POST" action="{{ $goal_status === 0 ? url('/mentee/my_assignments/assignment_start/' . $detail->id ) : url('/mentee/my_assignments/assignment_start/' . $detail->id ) }}">
                            {{ csrf_field() }}
                            </br>
                            <button  type="submit" id="submit_btn" class="btn btn-success">{{ $goal_status === 0 ? 'Start Assignment' : 'Mark as Complete' }}</button>
                        </form>
                    </div>
                    @endif
                </div>
    ...
    <script type="text/javascript">
        ...
        $("#image").on("change", function() {
            if ($("#image")[0].files.length > 5) {
                ...
                alert("You can select only 5 images at a time.");
            } else {
