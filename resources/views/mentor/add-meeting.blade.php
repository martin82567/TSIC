            ...
            <div class="box-inner">
                <div class="form-section">
                    ...
                    <div class="row">                                                
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="description"  id="description" <?php if(!empty($meeting_requests)){?> readonly <?php }?>><?php echo !empty($meeting_data->description)?$meeting_data->description:'';?></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row"> 
                            ...
                            <div class="form-group">
                                ...
                                <div class="select">
	                                <select name="mentee_ids" id="mentee_ids" <?php if(!empty($meeting_requests)){?> disabled <?php }?>>
                                        ...
                                        <option value="<?php echo $m->id; ?>" <?php if(!empty($meeting_data->mentee_id) && $meeting_data->mentee_id == $m->id){?> selected <?php }?> data-school-id="<?php echo $m->school_id; ?>" data-school-name="<?php echo $m->school_name; ?>" ><?php echo $m->firstname.' '.$m->lastname;?></option>
                                        <?php }}?>
	                                </select>
	                            </div> 
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="school_location"  id="school_location" style="height: 40px;" <?php if(!empty($meeting_requests)){?> readonly <?php }?>><?php echo !empty($meeting_data->school_location)?$meeting_data->school_location:'';?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="box-footer">
                        ...
                        <a href="{{ url('/mentor/meeting/list?type=requested') }}" class="btn btn-danger">Cancel</a>                       
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if(id != '0'){
	        ...
	        $.ajax({
	            url: "{{ url('/mentor/meeting/get_session_data_ajax') }}",
	            type: "POST",
	            data: {
	               id: id,
	               _token: "{{ csrf_token() }}"
	            ...
	            success: function(data) {                
                    var session_data = jQuery.parseJSON(JSON.stringify(data.session_data)); 
                    var school_name = jQuery.parseJSON(JSON.stringify(data.school_name)); 
                    var school_type = data.session_data[0].school_type;
