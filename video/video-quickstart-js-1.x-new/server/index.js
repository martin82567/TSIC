...
['bandwidthconstraints',
  ...
  'datatracks',
  'manualrenderhint',
  'autorenderhint'].forEach(function (example) {
...
io.on('connect', function onConnect(socket) {
  ...
  socket.on('endBeforeReceived', function (data) {
    ...
    if (data.denied_by == "app") {
      io.to('room_' + data.sender_type + '_' + data.sender_id).emit('endVideo', { endBefore: true });
    }
